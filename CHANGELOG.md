...
- **Status badge now follows the operation**: instead of sitting at the top
  centre of the screen (easy to miss), the badge appears right above the
  element being acted on and stays 1.8s — it literally tracks what the AI is
  touching. Falls back to top-centre when no coordinates are known.

## 0.1.43

- **Foreground protection auto-wired into ui.click and keyboard.write**: after
  every background delivery the plugin checks whether the target app stole the
  foreground and, if so, hands it back via oreground restore (ALT trick in
  a fresh process). Result fields: oregroundRestored, originalForeground.
  E2E verified both paths (Settings click and search-box write).

## 0.1.42

- **Panel auto-wiring**: every background mutate (ui.click, mouse.click,
  keyboard.write, mouse.scroll) now drives the task panel automatically —
  first operation opens it, each step updates the description and progress,
  and 6 seconds of idle closes it. No caller changes needed.
## 0.1.41
